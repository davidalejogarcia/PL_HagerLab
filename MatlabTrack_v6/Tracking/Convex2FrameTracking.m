function [Tracks,TrkPtsAdded] = Convex2FrameTracking(Particles,varargin)
...
%cost, and the last timepoint the particle was found
T = [T, NaN(size(T)),zeros(size(T,1),2),tpoints(1)*ones(size(T,1),1)];
maxInd = size(T,1);
...
for i = 1:size(Pi,1)
    for j = 1:size(T1,1)
        ...
        if costMat(i,j) > Gd || (Pi(i,ndimT1+1) - T1(j,2*ndimT1+3)) > gaps
            costMat(i,j) = Inf;
        end
