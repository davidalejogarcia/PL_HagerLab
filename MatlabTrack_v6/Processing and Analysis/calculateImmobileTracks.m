function [filTracks, allFilTracks] = calculateImmobileTracks(Tracks, ThresholdL, minFrame, ThresholdH, plotFlag);
...
if size(Tracks,2) == 5 || size(Tracks,2) == 7
    partInd = 4;
elseif size(Tracks,2) == 6 || size(Tracks,2) == 8
    partInd = 5;
end
    
for i = 1:max(Tracks(:,partInd));         
    idx = find(Tracks(:,partInd) == i); 
...
%          jd_temp = sqrt((trackTemp(2:end,1)- trackTemp(1:end-1,1)).^2 + ...
%                     (trackTemp(2:end,2)- trackTemp(1:end-1,2)).^2 + ...
%                     (trackTemp(2:end,3)- trackTemp(1:end-1,3)).^2);
%      end
     ...
     for j = 1: length(jd_temp)
         ...
         if jd_temp(j) < ThresholdL && ~previous
             ...
             catTrack = trackTemp ([j j+1],1:(partInd-1));
             catTrack(:,partInd) = particleIx;
             catTrack(:,partInd+1:partInd+3) = trackTemp ([j j+1],partInd+1:partInd+3);
             catTrack(:,partInd+4) = trackTemp([j j+1], partInd);
             allFilTracks = [allFilTracks ; catTrack];
    
             
         elseif jd_temp(j) < ThresholdL  && previous
             catTrack = trackTemp (j+1,1:(partInd-1));
             catTrack(:,partInd) = particleIx;
             catTrack(:,partInd+1:partInd+3) = trackTemp (j+1,partInd+1:partInd+3);
             catTrack(:,partInd+4) = trackTemp (j+1,partInd);
             allFilTracks = [allFilTracks ; catTrack];
...
if  ~isempty(filTracks) && ThresholdH > 0; 
%% Identify Tracks that jump less than ThresholdH between minFrame frames

   
    for i = 1:max(filTracks(:,partInd));
    
        idx = find(filTracks(:,partInd)== i);
...
%             Test = sqrt((trackTemp(minFrame:end,1)- trackTemp(1:end-minFrame+1,1)).^2 + ...
%                     (trackTemp(minFrame:end,2)- trackTemp(1:end-minFrame+1,2)).^2 + ...
%                     (trackTemp(minFrame:end,3)- trackTemp(1:end-minFrame+1,3)).^2);
%         end
...
TestAccum = [];
Test = filTracks(2:end,partInd) - filTracks(1:end-1,partInd) > 0;
for i = 1:length(Test);
    TestAccum(i) = sum(int16(Test(1:i)));
end
TestAccum = [1, TestAccum + 1];

filTracks(:,partInd) = TestAccum;
