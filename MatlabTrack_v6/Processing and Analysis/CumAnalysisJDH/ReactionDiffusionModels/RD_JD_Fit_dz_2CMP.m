function [FitPar, FitParSigma, FitSSR, Fit] = RD_JD_Fit_dz_2CMP(Data, Par, Ini, plotFlag)
...
% Par{1} = tlist
% Par{2} = rlist
% Par{3} = Db, diffusion coefficient of bound molecules
% Par{4} = sigma, localization accuracy
% Par{5} = [D1,D2], diffusion coefficients for the two components
%
...
FitParLB = [0,  0, 0, 0,  0.4];
% define upper boundaries for the fitted parameters
FitParUB = [ 1, Inf, Inf, Inf, 10];

% prepare function for fit
fitfun = @(COEF) (RD_JD_Fun_dz_2CMP(COEF, Par) - Data);
...
Fit = RD_JD_Fun_dz_2CMP(FitPar, Par);
...
if plotFlag ~= 0
 ...
 figure;
    ...
    title({'Jump Histogram Distribution - Full Model Fit',...
        ['D1 = ', num2str(Par{5}(1),3), '\mum^2/s, D2 = ', ...
        num2str(Par{5}(2),3), '\mum^2/s'],...
        ['k_{on} = ', num2str(FitPar(3),3), 's^{-1} k_{off} = ', num2str(FitPar(4),3), 's^{-1}'],...
        ['dZ =', num2str(FitPar(5),3)]})
...
disp(['D1 = ', num2str(Par{5}(1),2), '\mum^2/s'])
disp(['D2 = ' num2str(Par{5}(2),2), '\mum^2/s'])
