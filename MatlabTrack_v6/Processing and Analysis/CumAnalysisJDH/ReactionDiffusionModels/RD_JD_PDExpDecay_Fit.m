function [FitPar, FitParSigma, SSR, Fit] = RD_JD_PDExpDecay_Fit(Data, Par, Ini, plotFlag)

% ReactionDiffusion_JD_Fit
% ------------------------------------------------------------------------
% Fit a Reaction diffusion model to the Two-dimensional Jump Distance
% Histogram
%
...
FitParUB = [Inf, Inf,Inf,  10];

% prepare function for fit
fitfun = @(COEF) (RD_JD_PDExpDecay_Fun(COEF, Par) - Data);
...
Fit = RD_JD_PDExpDecay_Fun(FitPar, Par);
...
if plotFlag ~= 0
 ...
 figure;
    ...
    title({'Jump Histogram Distribution - Full Model Fit',...
        ['D = ', num2str(FitPar(1),2), 'k_{off} = ', num2str(FitPar(3),3), 's^{-1}, \sigma = ', num2str(FitPar(4),4), 'mum']});
...
disp(['koff = ', num2str(FitPar(3),2), 's^{-1}'])
disp(['sigma = ', num2str(FitPar(4),2), 'mum'])
disp('_________________________________________')
