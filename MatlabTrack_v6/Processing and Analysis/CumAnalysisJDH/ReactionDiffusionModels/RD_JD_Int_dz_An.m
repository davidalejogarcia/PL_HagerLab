function Integrand = RD_JD_Int_dz_An(s,t, rlist, COEF)
...
% Term1 = (rlist.^(fractalD - 2))./s.^(fractalD/walkerD);
% Term2 = (rlist./s.^(1/walkerD)).^(walkerD/(2*(walkerD - 1)) - fractalD);
% Term3 = exp(-(rlist./(4*D*s.^(1/walkerD))).^(walkerD/(walkerD-1)));

Diff = quadv(@(z)DiffProp3D_serpinski(rlist, s, z, D, gamma),-10,10);
