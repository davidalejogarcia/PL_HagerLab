function [FitPar, FitParSigma, FitSSR Fit] = RD_JD_Fit_dz_freeDb(Data, Par, Ini, plotFlag)

% ReactionDiffusion_JD_Fit
% ------------------------------------------------------------------------
% Fit a Reaction diffusion model to the Three-dimensional Jump Distance
% Histogram. The bound component is allowed to diffuse with a fixed
...
FitParUB = [Inf, Inf, Inf, Inf, 10, Inf];

% prepare function for fit
fitfun = @(COEF) (RD_JD_Fun_dz_FreeDb(COEF, Par) - Data);
...
Fit = RD_JD_Fun_dz_FreeDb(FitPar, Par);
...
if plotFlag ~= 0
 ...
 figure;
    ...
    title({'Jump Histogram Distribution - Full Model Fit',...
        ...
        ['dZ =', num2str(FitPar(5),3), '\mum Db = ',num2str(FitPar(6),3), '\mum^2/s']})
